...
YIO_INTEGRATION_HOMEASSISTANT_DEPENDENCIES = qt5base qt5connectivity qt5tools qt5websockets yio-integrations-library
...
define YIO_INTEGRATION_HOMEASSISTANT_CONFIGURE_CMDS
    ...
    @mkdir -p $(@D)/build
    (cd $(@D)/build; $(TARGET_MAKE_ENV) YIO_BIN=$(@D)/bin YIO_SRC="$(STAGING_DIR)/usr/include/yio" $(HOST_DIR)/bin/qmake $(@D) $(YIO_INTEGRATION_HOMEASSISTANT_QMAKE_ARGS) "YIO_BUILD_VERSION = $(YIO_INTEGRATION_HOMEASSISTANT_VERSION)")
